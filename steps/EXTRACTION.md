...
> 2. Mom Question · 6. Problem Statement Wall · 7. Job/Pain/Gain Map ·
> 3. Laddering · 9. Socratic Probe · 10. Cognitive Interview
...
**Trigger:** any of (a) stated =/= revealed gap, (b) tacit layer is large, (c) user scores "unsure" in SERIOUSNESS heuristics, (d) 2+ working hypotheses won't converge, (e) answers sound guarded or face-saving, (f) a key term has 2+ live readings or smells cultural/dialectal/jargon.
...
|> **Truth-elicitation (for guarded or face-saving answers):** detecting lies is a dead end (Bond & DePaulo 2006: 54% accuracy across 206 studies, 24,483 judges) — elicit instead. (1) **Narrative before checking:** get the full account first, verify against evidence after; late/gradual surfacing beats early confrontation (SUE: Hartwig & Granhag; Oleszkiewicz & Watson 2020 meta-analysis). (2) **Contradiction as curiosity:** "earlier you said X, now Y — help me reconcile," then ask for _their_ explanation (SUE-Confrontation/Explain). Never accuse; explanation-requests raise disclosure, accusations raise resistance. (3) **Face-saving framing:** forgiving wording is the most effective disclosure method across 121 experiments (2026 systematic review, Quality & Quantity). (4) **Concrete over abstract:** ask "what happened last time X broke?" not "what are your constraints?" — abstract questions trigger ideal-self reflection even with no audience (Brenner & DeLamater). (5) **State verification openly:** saying "I'll check this against Y" increases forthcomingness (Luke et al.). (6) **Open narration first:** let them tell it uninterrupted before probing — free recall yields more detail without confabulation (cognitive interview: Fisher & Geiselman). (7) **Ask for checkable details:** "what would the logs / the invoice / the user say?" — truth-tellers offer verifiable detail, deceivers stay vague (verifiability approach: Nahari, Vrij & Fisher). (8) **One unanticipated angle:** a probe from a direction they didn't rehearse ("what did you decide NOT to include?") — planned accounts handle expected questions; unplanned ones separate recall from construction (cognitive-load tradition: Vrij et al.). (9) **No pressure, ever:** coercive tactics manufacture agreement, not truth — hundreds of documented false confessions came from pressure, not evidence. Elicitation is not interrogation; if answers stay guarded after (1)–(8), log the gap in the Assumption Ledger and move on. (10) **Adapt, don't script:** late/gradual beats early, but the advantage carries small-study-effects caution (Oleszkiewicz & Watson 2020) — run the sequence against the respondent in front of you, not the template; practitioner guidance converges on adaptive over rigid use.

> **Readings gate (for ambiguous or culture-loaded terms):** no single reading passes unconfirmed when stakes are high. **Stakes test:** the gate fires when the term is load-bearing — getting it wrong would change files, scope, or direction. Trivia passes; load-bearing terms don't. (1) **Enumerate:** list every live reading ("here are the 3 things this could mean") — ambiguity costs reanalysis downstream, diagnostic cues aid retrieval; enumerate early (SAT reanalysis cost +250ms when meaning must be revised). (2) **Pin the domain:** at minimum establish which domain the user means — domain first, precision after. (3) **Confirm:** the user picks; log pick + discarded readings in the INBOX flagged-terms table. (4) **Culture-flag, never normalize:** slang, memes, dialect, org-jargon, offline-culture terms get flagged and asked about, not silently inferred — models systematically rewrite dialects into standard forms (AAE→SAE silent correction, arXiv:2607.06845) and underuse/misuse dialect features (arXiv:2602.21485). High-context speakers leave more unsaid by default (Hall: same words, different interpretations across cultures) — probe their tacit layer harder, not lighter. (5) **Grounding criterion:** move on only when both sides can state the agreed reading back — mutual belief, not silent assent (grounding = presentation + acceptance; least effort is the enemy when error cost is high — Clark & Brennan). (6) **Flat-consensus escalation:** when no reading clearly wins — agent confidence flat across candidates — stop picking and hand the choice to the human with the annotated options. Cultural annotations plus targeted human moderation beat LLM-only judgment where models are weakest (C3MOD pipeline 78% vs GPT-4o 71%, arXiv:2503.07237): the agent's job is to annotate, the human's to resolve non-consensus.

### Context declaration (load-bearing terms and references)

> **Rule:** a load-bearing term, number, decision, or reference is declared on first use — **term · domain · one-line definition · prerequisites it presumes · what it contrasts with · why here**. A term is unusable without the chain it sits in: a reader who was never told what "regular" presumes cannot judge the claim that uses it. A pronoun is safe only when exactly one antecedent is live; an overloaded "it" / "this" / "they" is a declaration failure. **Scope:** load-bearing only — declaring everything is the over-constraint failure the prompt standard warns about (`docs/PROMPT_STANDARDS.md` rule 2). **Counterexample:** quoted text keeps its own wording, and a term already declared earlier in this run is not re-declared. **Check:** REVIEW 4.13 — a load-bearing term or reference used without its context is a FAIL; a missing declaration is not greppable, so the check is a reader test.
