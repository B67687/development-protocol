...
> **Brownfield rebuilds also produce the incumbent decision inventory.** The reanchor pass recovers DIRECTION; the incumbent's technical decisions and their reasons are recovered at EXECUTOR § Migration Protocol step 2 (KEEP/DROP table) before code is replaced.
...
| ------------------------------ | ----------------------------- | ------------------------------ | ----------------------------------- |
|                                | Knowledge (novice→expert)     | asks basics, vague terms       | precise terms, cites tradeoffs      | leads with specifics → thin verification pass; led-by + vague → full funnel + extra verification (novices over-rely and cannot self-verify — arXiv:2505.08063); high trust ≠ good discrimination — literacy doesn't fix over-reliance (arXiv:2604.01114v3), and experts under-rely when they misjudge difficulty (metacognitive failure, N=2,799) — calibrate trust behaviorally (verification + Dissent Log), not by credentials; calibrate guidance too — novices need full structure (worked examples), experts need a light touch (excess guidance backfires — expertise-reversal effect, CLT) |
|                                |                               | Assertiveness (defers→insists) | "you decide", revises fast          | "just do X", reverts edits                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | confident + detailed-but-unverified = highest-risk flag (such rebuttals persuade even when wrong — Kim & Khashabi EMNLP25; authority tone triggers most regressive sycophancy — SycEval); insistence against evidence → dissent log, owner's risk; guarded insistence → EXTRACTION truth-elicitation rules, never head-on challenge |
|                                | Attachment (exploring→locked) | "what should this be?"         | "I already decided"                 | locked → P2b still runs, framed as validation not choice (locked is often face-saving, not certainty — challenge the idea, never the person; ELEPHANT, SDB review 2026)                                                                                                                                                                                                                                                                                                                                                                                                                            |
| Patience (sprint→deliberative) | "quick", "today", "MVP"       | "thorough", "get it right"     | sprint → Light; deliberative → full |
|                                |                               | Reflectiveness (guarded→open)  | one-liners, deflects probes         | volunteers constraints, admits unknowns                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | guarded → accept + verify later (models avoid challenging users, +45pp face-preservation — ELEPHANT); open → probe now; never anchor a guarded user with the template early — raw-pass first (Einstellung, first-example fixation)                                                                                                  |
|                                | Stopping (ships→perfects)     | "good enough"                  | "one more thing" loop               | perfects → explicit done-criteria at STRATEGY, no open loops; ambition capped one scale-step per cycle (second-system effect — Brooks 1975; REFLECT Q9)                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
**Stance-table sources:** novice reliance arXiv:2505.08063 · assertiveness/sycophancy Kim & Khashabi EMNLP25, SycEval AAAI26, shared-circuit arXiv:2604.19117 · forcing Buçinca 2021 (reduces over-reliance, needs NfC), Stanford 2023 (forcing also cuts correct reliance — keep dissent cheap), partial explanations Monash 2025 · face-preservation ELEPHANT 2025, SDB systematic review 2026 (covers Attachment-locked) · ambition cap Brooks 1975 second-system (covers Stopping-perfects) · trust-vs-reliance arXiv:2604.01114v3 (literacy no fix — calibrate behaviorally), SSRN under-reliance (experts misjudge difficulty). Patience row remains a practitioner heuristic, no direct literature found.
