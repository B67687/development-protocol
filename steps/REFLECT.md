...
- **Learned-this-cycle:** 2-3 things the human should now be able to explain unaided — retrieval targets for the next cycle's explain-back. Gates are learning reps only if the learning is named. Count the learning signal too: (a) dissents/overrides logged, (b) unprompted pointed questions, (c) explain-backs passed first-attempt, (d) tripwire firings (negative), (e) spikes run this cycle (play cheapness: generation before instruction, counted here) — write the five counts in the seed, trend them against the prior cycle: rising a–c with falling d is learning; flat zeros with firings is abdication deepening; rising e with low cost is healthy play, 0 or runaway e is the signal.
  **Explain-back rubric (a pass is 2 sentences):** (1) names the decision in the human's own words — not the agent's verbatim phrasing; (2) states why it was chosen over at least one rejected alternative. Verbatim echo, or no reason given, is a fail. **Fading is bound to these counts:** two clean cycles (a–c rising, d at zero) → fade one level of the standing-decisions display (STRATEGY → scaffolding ladder).
