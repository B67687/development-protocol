...
| CAT-EXPLAINER-01 | EXPLAINER (folded into REVIEW) | Doc-only change with zero code artifacts to explain (record the scope bound) |
| CAT-SKIP-01 | Pipeline step (Light run) | Run-Shape Selector classified the run Light; the step is size-routed out per ../steps/INBOX.md — record which step |
| CAT-SKIP-02 | SERIOUSNESS | Protocol-improvement recursion (Invariant 4) — running the protocol on itself exempts SERIOUSNESS |
| CAT-SKIP-03 | DECOMPOSITION depth / cross-probe (MULTI folded into FUNDAMENTALS) | Level-of-Care 1: trivially reversible change; full decomposition/cross-probe unnecessary (record care level) |
